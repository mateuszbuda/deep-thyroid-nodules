...
def fold_pids(fold, test=True):
    ...
    for f_path in all_files:
        ...
        if (test and pid in val_ids) or (not test and pid not in val_ids):
            ...
            a = np.argmax(df_compos[df_compos.ID == pid].as_matrix().flatten()[1:].astype(np.float32))
            #if(a >= 2):
	    pids.append(pid)
...
def test_data():
	...
	print("DF COMPOS ID: ", df_compos.ID)
...
def fold_data(fold):
	...
	#TODO: CHANGED THE LINE BELOW
        a = np.argmax(df_echo[df_echo.ID == pid].as_matrix().flatten()[1:].astype(np.float32))
	#a = np.argmax(df_compos[df_compos.ID == pid].as_matrix().flatten()[1:].astype(np.float32))
	...
	#print("new im shape: ", image.shape)
		...
		#TODO: original --> y_test_compos.append(df_compos[df_compos.ID == pid].as_matrix().flatten()[1:].astype(np.float32))
            
            print("DF ECHO: ", df_echo[df_echo.ID == pid])
            print("YEET: ", df_echo[df_echo.ID == pid].as_matrix().flatten()[1:].astype(np.float32).shape)
    ...
    y_train = {
        "out_cancer": np.array(y_train_cancer),
        #"out_compos": np.array(y_train_compos)[:,2:5],
        "out_echo": np.array(y_train_echo),
        "out_shape": np.array(y_train_shape),
        "out_calcs": np.array(y_train_calcs),
        "out_margin": np.array(y_train_margin),

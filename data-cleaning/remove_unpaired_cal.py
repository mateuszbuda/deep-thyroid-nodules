...
filenames = sorted(
    glob("/media/maciej/Thyroid/thyroid-nodules/detection/Calipers/*.csv")
)
images_dir = "Nodules"
...
for fid in sorted(fid2fnames):
    ...
    if not (
        sorted((cals0_len, cals1_len)) == [2, 4]
        or sorted((cals0_len, cals1_len)) == [4, 4]
        or sorted((cals0_len, cals1_len)) == [2, 2]
    ):
        unpaired_cnt += 1
        for fname in fid2fnames[fid]:
            print("Removing {}".format(fname))
            os.remove(fname)
            image_fname = fname.replace("detection/Calipers", images_dir).replace(
                "csv", "PNG"
            )
